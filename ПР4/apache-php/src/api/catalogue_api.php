...
function addItem()
{
    ...
    $toyCost = $data['cost'];
    $mysqli = openMysqli();
    $result = $mysqli->query("SELECT * FROM games WHERE title = '{$toyName}';");
    if ($result->num_rows === 1) {
        $message = $toyName . ' already exists';
        outputStatus(1, $message);
    }
    else {
    $query = "INSERT INTO games (title, description, cost)
...
function removeItemByName()
{
    ...
    $result = $mysqli->query("SELECT * FROM games WHERE title = '{$toyName}';");
    if ($result->num_rows === 1) {
        $query = "DELETE FROM games WHERE title = '" . $toyName . "';";
        $mysqli->query($query);
        $mysqli->close();
...
function updateItemCostByName()
{
    ...
    $result = $mysqli->query("SELECT * FROM games WHERE title = '{$toyName}';");
    if ($result->num_rows === 1) {
        $query = "UPDATE games SET cost = " . $toyCost . " WHERE title = '" . $toyName . "';";
        $mysqli->query($query);
        $mysqli->close();
...
function getItemByName()
{
    ...
    $mysqli = openMysqli();
    $toyName = $_GET['name'];
    $query = "SELECT * FROM games WHERE title = '{$toyName}';";
